...
export class SnapshotEditService {
    ...
    public async finalize() {
        this.logger.info("Finalizing edit session");
        // ... do work ...
        this.logger.info("Edit session finalized");
    }
}
```
...
28. **Canonical observability fields are camelCase, grouped, and atomic.** Every log line, Sentry tag, and PostHog property should carry the same canonical IDs. The schema lives in `packages/logger/src/observability-context.ts` (`ObservabilityContextSchema`) and is organized into atomic groups: `temporal`, `organization`, `application`, `branch`, `snapshot`, `testCase`, `testGeneration`, `run`, `job`, `compaction`, `preview`. Each group is optional but its required fields are mandatory - you can't have a `snapshot` group with `headSha` but no `snapshotId`. At an entry point (Temporal activity interceptor, job entry, request handler) wrap the work in `withObservabilityContext({ snapshot: { snapshotId }, ... })`; downstream code calls `extendObservabilityContext({ branch: { branchId } })` as more groups become known. Inside that scope, every `logger.*` call automatically carries those fields, flattened to top-level keys at emit (snapshotId, branchId, workflowId, ...) - do **not** thread IDs through `logger.child({ snapshotId, ... })`. The `extra:` bag is the only place for non-canonical fields: `logger.info("...", { extra: { count: 42 } })`. Add new fields to the appropriate group (or create a new group) before using them; never invent ad-hoc keys at call sites or use `snake_case` field names.
29. **UI-changing PRs carry screenshots.** When a PR visibly changes the UI (apps/ui or packages/blacklight), use the `ui-screenshots` skill: write or extend a Storybook story with MSW fixtures typed against `RouterOutputs`, screenshot it headlessly (`storybook:shoot`), upload to S3, and embed the image in the PR description. Never commit the PNGs - they are LFS-tracked and the output dir is gitignored.
30. **Prefer tRPC polling over push transports for UI liveness.** When the UI needs to observe changing backend state, poll a tRPC query with React Query's `refetchInterval` (add `refetchIntervalInBackground: true` when a backgrounded tab must keep observing). Never add SSE/WebSocket routes for UI state - streaming transports are fragile behind nginx/CloudFront (buffering, idle timeouts, credentialed CORS) and add infra surface for no gain at our poll rates. The previewkit build/app log streams are the one grandfathered exception; do not add new ones.
31. **Never rely on the nginx `/v1/` same-origin redirect; always address the API by its `VITE_API_URL` hostname.** Frontend code that talks to the API must build its URL from `env.VITE_API_URL` (e.g. `` `${env.VITE_API_URL}/v1/...` ``), never a bare same-origin `/v1/...` path that depends on the nginx proxy in `apps/ui/nginx.conf.template`. That `/v1/` `location` block exists only as a fallback; new call sites must use the `VITE_API_URL` hostname directly.
32. **Always create PRs with a Conventional Commits title.** Every `gh pr create --title ...` must follow `<type>(<scope>): <description>` (e.g. `feat(previewkit): add build caching`, `fix(ui): correct login redirect`, `chore(engine): bump appium version`). Use the repo's recent PR titles (`git log --oneline` on `main`) to pick the right `type` and `scope` for the area being changed.
33. **The root `README.md` is the public front page - keep its screenshots and its claims true.** It is mirrored to the public repo `Autonoma-AI/autonoma` and embeds two **committed** screenshots of the real UI: `.github/assets/pr-review.webp` (story `pages-authoritativeprpage--report`) and `.github/assets/analysis-issue.webp` (story `pages-authoritativesnapshotpage--issue`). When a PR changes either of those pages, re-shoot the affected image and commit it in the same PR - the `ui-screenshots` skill has the exact stories, viewports, and commands. Prose goes stale the same way: when a change alters what the product does or how you set it up, update the README alongside `apps/docs/`, and never describe capability the shipped product does not have (the GitHub PR-review path is web-only). Images under `.github/assets/` are WebP and deliberately outside LFS - never add a `.png`/`.jpg` there, it would reach the public mirror as an LFS pointer.
34. **Docs screenshots are part of the docs - update them with the prose.** `apps/docs` uses three image tiers: a generated atmospheric **hero**, a generated labeled **diagram**, and a product **screenshot** (`apps/docs/public/img/<section>/*.png`) answering "what is the actual thing I will operate". When a change alters a screen the docs picture, re-shoot the affected image in the same PR - a stale screenshot is worse than none, because it teaches a UI we no longer ship. `apps/docs/screenshot-plan.md` maps every image to the Storybook story and viewport that produced it and records what is deliberately *not* worth shooting; the `ui-screenshots` skill has the mechanics. Four rules that are easy to get wrong: start Storybook with `VITE_API_URL=https://api.autonoma.app` or the shot bakes in `localhost:4000`; check the frame for credential-shaped fixture values before publishing; confirm the component is not clipped at the bottom of the viewport; and pad the image to `#0a0a0a` scaled to its rendered width, so it frames the content rather than cropping it. Alt text is the whole value of the image to an agent - `llms.txt` is generated at build time and an image degrades to its alt text there, so describe the labels, states and numbers on screen, never "screenshot of the config page".
