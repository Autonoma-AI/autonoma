...
resource "aws_vpc_endpoint" "main_aps_workspaces" {
  vpc_id              = aws_vpc.main.id
  ...
  security_group_ids = ["sg-048624221709749ef"]
}
...
resource "aws_vpc_endpoint" "previewkit_aps_workspaces" {
  vpc_id              = aws_vpc.previewkit.id
  service_name        = "com.amazonaws.us-east-1.aps-workspaces"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  security_group_ids = ["sg-0e74f310e87ab0f90"]
}
