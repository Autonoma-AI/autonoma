...
- **A recipe edit is only live on the snapshots it was written to.** Recipe versions are keyed by `(scenarioId, snapshotId)` and the provisioning path resolves the version pinned to the *run's* snapshot - `scenario.activeRecipeVersionId` never participates. Since snapshots are cloned per PR and per deploy, `applyScenarioRecipeUpdate` writes main's active snapshot (plus main's pending one) and retargets the active pointer to main's active-snapshot version. Do not add a write path that only touches `activeRecipeVersion`: the pointer is set at scenario creation and, without retargeting, stays on the snapshot the app was onboarded on - which goes `superseded` on the first deploy, stranding the edit where nothing reads it. Feature branches, investigation twins and merge proposals are deliberately NOT written: a branch snapshot is the recipe its runs were judged against. The consequence worth knowing when debugging: **an already-open PR does not pick up a recipe fix**, not even on a push, because a new branch snapshot forks from the branch's own active snapshot rather than main's.
