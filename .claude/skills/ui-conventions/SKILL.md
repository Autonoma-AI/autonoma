...
`apps/ui` ships a Storybook with MSW network mocking, so any component or full page renders from fixtures - no backend, database, or onboarding. Support code lives in `src/lib/storybook/`: tRPC fixtures typed against `RouterOutputs` (`trpcHandler`), better-auth mocks (`authHandlers`), the app-shell baseline (`appShellHandlers`), and `PageStory` for rendering real routes. Stories live in `src/stories/` or co-located with components - NEVER under `src/routes/` (the router plugin scans that tree). Screenshot them headlessly with `pnpm --filter @autonoma/ui storybook:shoot --story <id>` - take the whole boot-wait-shoot-kill block from the `ui-screenshots` skill rather than backgrounding `pnpm --filter @autonoma/ui storybook` yourself, or the dev server outlives your turn.
