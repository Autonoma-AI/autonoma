...
`settleAnalysisGitHub` (`apps/workers/diffs/src/activities/analysis`) invokes `postAnalysisComment` through the shared comment system after a successful, durable run, flag-gated OFF by `ANALYSIS_PR_COMMENT_ENABLED`. The comment is **issues-first**: the cards are the branch's OPEN bug issues, not this snapshot's findings, so a bug carried across snapshots keeps its card and each one deep-links to the stable PR-level issue-detail page. Its two links mean different things - the title goes to the ISSUE, the media and "Watch replay" go to the ONE RUN the issue's primary test designated (the animated clip is preferred over the issue's hero frame, since GitHub does not render the hero's pin). The card **diagnoses** rather than prescribes: a "Suspected cause" line plus a nested Evidence collapsible built from the repo-validated `codeReferences`, and no fix instructions.
