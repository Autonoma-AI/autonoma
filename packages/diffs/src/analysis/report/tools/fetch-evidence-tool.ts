import { AgentTool, type AgentToolModelOutput, type AgentToolModelOutputOptions, imageToolContent } from "@autonoma/ai";
import { z } from "zod";
...
type FetchEvidenceOutput = { assetId: string; label: string; base64: string; mediaType: string };
...
export class FetchEvidenceTool extends AgentTool<FetchEvidenceInput, FetchEvidenceOutput, ReporterAgentLoop> {
    ...
    protected async execute({ assetId }: FetchEvidenceInput, loop: ReporterAgentLoop): Promise<FetchEvidenceOutput> {
        const { label, base64, mediaType } = await loop.fetchEvidence(assetId);
        return { assetId, label, base64, mediaType };
    }
