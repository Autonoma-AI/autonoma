...
- engine_artifact: the harness hit a wall it DEFINITIVELY cannot get past on a step that is REASONABLE for a browser test, and that wall BLOCKED the run - the step was right, the agent physically could not perform it, and the failure was TERMINAL (the run could not get past it). The canonical cases: native browser chrome (\`confirm\`/\`alert\`/\`prompt\`, the file picker, basic-auth popups - not DOM-clickable, so a dialog-gated action that never fires is engine_artifact, not a bug), and a capability the harness genuinely lacks (audio/voice interaction, a second device, an email inbox). App is fine in EVERY engine_artifact - if the app misbehaved, this is the wrong category. It is NOT:
  ...
  * a plan step no browser test should ever contain ("run this script", "check the server logs", "open devtools") - that plan is flawed by design: bad_test, never engine_artifact;
  * the agent going to the wrong place because the written plan does not match the real UI (wrong tab, a missing/renamed/moved element, steps that don't fit the actual layout) - outdated_test or bad_test (fix the plan, emit suggestedTestUpdate);
  * an assertion that fails because the app's OWN designed behavior moves the UI out of the asserted state - content that intentionally auto-hides, expires, collapses, or re-masks after an interaction. The agent drove every step fine and the app worked AS NOW DESIGNED; the plan asserts a state the app no longer settles in. That is outdated_test. A "race" counts as engine_artifact ONLY when it is harness noise - never when the timing IS the app's intended behavior (check the diff for a timer/expiry/animation the PR introduced before calling any failed-assertion a flake);
...
- outdated_test: the app WORKS (as now intended) but the test no longer matches it. The test was valid for the OLD app - it passed before, or its steps plainly fit the previous UI - and this PR's intentional change made it stale: a moved/renamed element, changed copy, a flow that gained or lost a step, OR changed timing/lifecycle behavior (the asserted content now auto-hides, expires, or renders differently). The fix is TEST-side (re-record/rewrite via suggestedTestUpdate), never an app fix. REQUIRES the app to have rendered: if the content the test needed never loaded, the app did NOT work - that is not outdated_test; find the app/data/env cause.
- bad_test: flawed BY DESIGN (asserts nothing meaningful) or asserts a feature that NEVER existed (no component, no i18n key, no git history) - INCLUDING a test that was wrong from genesis (it never passed and its steps/assertions never matched the app). Not for missing data the scenario should seed (scenario_issue), a control the agent couldn't drive (engine_artifact), or content that never rendered (an app/data/env failure - a never-loaded screen is not a test-design flaw).
...
- keyStepIndex: from the step-by-step trace above, the 1-indexed step number whose screenshot most clearly SHOWS this finding to a human opening the report - the single frame you would put in front of them. This is YOUR judgment, NOT mechanically the failed step: an assertion can be wrong, and the real defect (a broken page, wrong data, an error state) is often most visible a step before or after the failure. Use the video to locate the telling moment, then map it to the nearest trace step. Null only if no step frame is representative (then the final screenshot is used).
