...
- plan_mismatch: the app WORKED, but the test's PLAN does not match it - the run "failed" on the test, not the app. It is for a STALE plan (a test valid for the OLD app that an intentional change made stale) or an otherwise inaccurate plan WHEN you can write a concrete, meaningful, browser-executable replacement that preserves the test's user-facing intent. Emit that COMPLETE revised plan in suggestedTestUpdate. An empty suggestedTestUpdate is allowed only when the evidence is still insufficient to decide whether a viable plan exists; say exactly what you could not establish in planMismatchNote. REQUIRES the app to have rendered: if the content the test needed never loaded, the app did NOT work - that is not plan_mismatch; find the app/data/env cause. Not for missing data the scenario should seed (scenario_issue), a control the agent couldn't drive (engine_artifact), or content that never rendered (an app/data/env failure).
- invalid_test: the test's DESCRIPTION has no meaningful, viable browser-test rewrite and should be REMOVED. The split is MODEL-JUDGED recoverability: plan_mismatch has a specific replacement that would still test a real user-facing behavior; invalid_test does not. Common proof includes a feature/label/flow that does not and did not exist, structurally unexecutable steps ("run this SQL", "read the server logs"), a premise the app contradicts, an incoherent or duplicate instruction with no distinct behavior to preserve, or a sequence of attempted rewrites showing that every alternative would fabricate, empty out, or weaken the test's stated intent. A failed self-heal is evidence, NEVER an automatic deletion rule: YOU decide whether the accumulated description, run, code, and prior attempts establish that no meaningful plan remains. Do not require a literal historical nonexistence proof when the test description and observed app behavior already make the absence of a viable user-facing assertion clear; do require specific evidence and a clear explanation of why a concrete rewrite would not recover it. When genuinely uncertain, keep it as plan_mismatch. Fill invalidTestNote (the failure mode + why no meaningful rewrite remains) and falsePositiveRisk (the "could this actually be salvageable?" self-check). Not for a control the agent couldn't drive (engine_artifact), missing seeded data (scenario_issue), or a stale/rewriteable test (plan_mismatch).
...
- Always set headline and planFidelity. Set falsePositiveRisk for client_bug / environment_failure / scenario_issue / invalid_test; null otherwise. For a plan_mismatch, suggestedTestUpdate is the COMPLETE revised plan - or an EMPTY STRING only while the evidence is insufficient to decide whether a meaningful rewrite exists, never null - and planMismatchNote is always the post-mortem; for a passed run with non-exact fidelity emit suggestedTestUpdate to tighten it; on every OTHER category both are null. For an invalid_test, set invalidTestNote (why no meaningful rewrite remains); null otherwise.
...
- Set planFidelity (exact/partial/diverged). Set suggestedTestUpdate to the COMPLETE revised plan for a plan_mismatch (fix the wrong assertion/step - even at exact fidelity) OR whenever fidelity is NOT exact and the feature exists/was verified (INCLUDING a passed run); otherwise null - except on a plan_mismatch where more evidence is needed before deciding whether a meaningful rewrite exists, when it is an EMPTY STRING, never null. On a plan_mismatch ALWAYS set planMismatchNote too (what the test asserted that was wrong, the rewrite tried, and why it still failed). For evidence fields file/lines/snippet, use null when not applicable.
- invalid_test vs plan_mismatch: choose plan_mismatch when you can write a concrete, meaningful browser-test plan that preserves the instruction's user-facing intent. Choose invalid_test when YOU conclude that no such plan remains: this can be proven by a nonexistent feature, browser-unexecutable steps, a contradicted premise, an incoherent/duplicate instruction, or failed rewrites that show every alternative would weaken, invent, or empty the intended behavior. A failed self-heal is evidence, NEVER an automatic deletion rule. Set invalidTestNote (the failure mode + why no meaningful rewrite remains) and falsePositiveRisk (could it be salvageable?) with >=1 specific evidence item. When uncertain, keep it as plan_mismatch.
...
function buildPriorPassSection(priorPass: NonNullable<ClassifierInput["priorPass"]>): string {
    ...
    return [
        ...
        "  and explain why in invalidTestNote with evidence. The number of failed repairs NEVER decides deletion; YOU",
