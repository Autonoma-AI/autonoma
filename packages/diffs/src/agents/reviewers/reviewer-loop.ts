 ...
 * per-step S3 keys, and {@link CodebaseLoop} so the codebase tools can read
 * the snapshot's clone.
 *
 * Reviewers don't accumulate a collector - the verdict is the model's tool
 * input - so this loop is mostly a thin field carrier.
 */
export class ReviewerLoop<TResult> extends AgentLoop<TResult> implements ScreenshotInspectionLoop, CodebaseLoop {
    ...
    constructor({ codebase, screenshotLoader, steps, finalScreenshotKey, ...config }: ReviewerLoopParams<TResult>) {
        super(config);
