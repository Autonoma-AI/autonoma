...
- outdated_test: the app WORKS but the recorded steps/targets no longer match the UI (a moved/renamed element), OR the PR intentionally changed the behavior the test asserts. Needs re-recording. REQUIRES the app to have rendered: if the content the test needed never loaded, the app did NOT work - that is not outdated_test; find the app/data/env cause.
...
- The execution agent's run result (pass/fail/steps/reasoning) is a HINT, not the truth - it optimizes to finish the test, not to audit the app, and is often wrong. The VIDEO + screenshots are the ground truth: form your OWN judgment from them. Always report confirmed app problems in observedAppIssues, independent of the test's outcome - and promote each DISTINCT one to a secondaryObservation so it becomes its own tracked finding, not a footnote on a test that may itself be wrong.`;
...
- secondaryObservations: the STRUCTURED, trackable breakdown of observedAppIssues - a list of DISTINCT app defects you VISUALLY CONFIRMED that are SEPARATE from this test's own assertion, each of which becomes its OWN finding in the report (so a real bug the test walked past is tracked on its own and can be reconciled across tests, not lost in a footnote). Use it for a clearly-visible defect unrelated to the verdict: a raw id or untranslated i18n key leaking into the UI, a broken image, an empty region where data is expected, a control that never rendered, or text that OVERLAPS other elements / spills outside its box and obscures neighbours. Each item: category (client_bug for a visible app defect, environment_failure for infra/config, scenario_issue for missing seeded data), confidence, a short headline, and detail (WHAT is wrong and WHERE). Be CONSERVATIVE - only distinct, clearly-visible, VERIFIED defects; do NOT restate the primary verdict here, and return an empty list if nothing else was visibly broken. TEXT OVERFLOW/CLIPPING IS THE #1 FALSE POSITIVE - do NOT emit an observation just because text is not fully visible: a long value that scrolls inside a text input / textarea / select, a long email/URL/name in a field, an ellipsis "..." truncation, or content clipped by a scroll container are ALL EXPECTED behaviour, never a bug. Only report overflow when text visibly ESCAPES its container and OVERLAPS/obscures other content, or a fixed non-editable label/heading/cell is cut off mid-content with meaning LOST and no scroll/expand affordance. When unsure whether an overflow is a real defect, LEAVE IT OUT.
