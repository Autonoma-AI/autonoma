...
export interface MintSecretKeyParams {
    ...
    /** Names the encryption key and is stamped into every envelope it seals, e.g. "1". */
    keyId: string;
}

/**
 * Creates an encryption key and promotes it to primary. An operator action, run
 ...
 * key named in their own envelope, so promoting a new primary is safe
...
export async function mintSecretKey({ db, provider, keyId }: MintSecretKeyParams): Promise<void> {
    ...
    logger.info("Minting a previewkit encryption key", { extra: { keyId } });

    const existing = await db.previewkitEncryptionKey.findUnique({ where: { id: keyId } });
    if (existing != null) {
        throw new Error(
            `Encryption key "${keyId}" already exists. Key ids are permanent - ` +
    ...
    // key seals its first value. The plaintext is discarded here.
    const validated = new SecretCipher(keyId, material);

    await db.$transaction(async (tx) => {
        await tx.previewkitEncryptionKey.updateMany({ where: { primary: true }, data: { primary: false } });
        await tx.previewkitEncryptionKey.create({
    ...
    logger.info("Encryption key minted and promoted to primary", { extra: { keyId } });
}
