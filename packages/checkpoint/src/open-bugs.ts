 ...
 * snapshot when one of its issues is linked (via a generation review or a run
 * review) to that snapshot. The `COUNT(DISTINCT bug_id)` is pushed down to SQL so
 * a bug surfacing across multiple reviews (or both review paths) counts once
 * without deduplicating in memory.
 *
...
export async function countOpenBugsBySnapshot(db: PrismaClient, snapshotIds: string[]): Promise<Map<string, number>> {
    ...
    // A bug reaches a snapshot through one of two review paths (generation review
    // or run review); we UNION both, then count distinct bug ids per snapshot.
    const rows = await db.$queryRaw(Prisma.sql`
        ...
        FROM (
            ...
            UNION ALL
            SELECT i.bug_id, tca.snapshot_id
            FROM issue i
            JOIN run_review rr ON rr.id = i.run_review_id
            JOIN run r ON r.id = rr.run_id
            JOIN test_case_assignment tca ON tca.id = r.assignment_id
            JOIN bug b ON b.id = i.bug_id
            WHERE b.status = 'open' AND tca.snapshot_id IN (${Prisma.join(snapshotIds)})
        ) linked
