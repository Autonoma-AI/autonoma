...
const findingSelect = {
    ...
    currentClassification: {
        select: {
            ...
            screenshotKey: true,
            error: true,
            // The plan and the recording keys are the generation's own facts - no longer written onto new rows and
            // read straight off the generation. `plan` follows the generation's immutable pinned `test_plan`, which
            // is exactly the plan the classifier judged.
            generation: {
                select: {
                    videoUrl: true,
        ...
        current:
            current == null
                ...
                : {
                      ...
                      plan: current.generation.testPlan.prompt,
                      ...
                      videoKey: current.generation.videoUrl ?? undefined,
                      optimizedVideoKey: current.generation.optimizedVideoUrl ?? undefined,
