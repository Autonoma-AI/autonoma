...
- **No retries.** Each SDK call is a single attempt. A network failure, non-2xx status, or schema validation failure throws immediately - the caller (job/onboarding state) decides what to do. Retrying a malformed response would just fail the same way, and the up/down jobs are the right place for any higher-level retry policy.
