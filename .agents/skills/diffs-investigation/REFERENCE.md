...
The merged analysis pipeline writes the same `analysis-conversation.json` key for its
Impact Analysis stage, plus `classify-{generationId}-conversation.json` per classified
run and `reporter-conversation.json`. There is no `diffs_job` row to read the URL off -
list the prefix instead:

```bash
aws s3 ls s3://autonoma-assets/diffs-job/<snapshotId>/
```

An absent `analysis-conversation.json` on a merged-pipeline snapshot means the upload
failed (it is best-effort) or the stage never reached the agent - not that the agent
selected nothing.
