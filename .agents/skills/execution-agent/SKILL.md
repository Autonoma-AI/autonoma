...
5. **Record step** - save the attempt with before/after metadata.
6. **Loop or stop** - continue until `execution-finished` is called or `maxSteps` (default 50) is reached.

**Loop detection**: when the model finishes with `success: false` and its reasoning mentions "loop", "stuck", "no progress", "repeating", or "repeated", the run is flagged as a loop.
...
│   ├── agent/          # execution-agent.ts, execution-agent-factory.ts, execution-result.ts, tools/, memory/
│   ├── runner/         # execution-agent-runner.ts, artifacts, events
│   ├── generation-api/ # API-driven runner + persistence
│   └── local-dev/      # local-runner.ts, test-case loading, cost summary
...
interface CommandSpec {
  ...
  params: object;        // what gets stored to execute the step
...
Every command is exposed to the LLM, so the directories under `commands/commands/` and the tools under `execution-agent/agent/tools/commands/` are one-to-one: `assert`, `click`, `drag`, `hover`, `navigate`, `read`, `refresh`, `save-clipboard`, `scroll`, `type`.
...
- `ClipboardDriver` - read the clipboard (backs `save-clipboard`).
...
- `StepAttempt<TSpec>` - a single command attempt, discriminated on `status`:
  - `GeneratedStep<TSpec>` (`status: "success"`) - execution output plus before/after metadata.
  - `FailedStep<TSpec>` (`status: "failed"`) - the attempted `interaction`, the raw model `input`, the extracted `params` (absent if extraction itself threw), `error`/`errorName`, and the before metadata. Failed attempts are recorded, not discarded - they make the agent's botched attempts visible to users and to review.
- `ExecutionResult<TSpec>` - full result: `generatedSteps` (the successful subset), memory, success, `finishReason: "success" | "max_steps" | "error"`, reasoning, final screenshot, conversation.
