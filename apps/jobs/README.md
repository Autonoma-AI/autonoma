...
|-----|-------------|---------|
| **run-completion-notification** | `@autonoma/job-run-completion-notification` | Billing + reaper only: `generation-exit <generationId>` fires the Stripe refund on a failed generation, `mark-failed <generationId>` flips a stuck generation to `failed`. PR results comments are posted by the analysis pipeline, not this job. |
