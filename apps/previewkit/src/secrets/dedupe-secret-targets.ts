/**
 * The identity of one secret bundle, which is what decides the K8s Secret it
 * writes: the app it belongs to (→ target Secret name) and the Application that
 * owns it (→ the bundle its values are read from).
 */
...
/**
 * Collapse secret bundles to one per K8s Secret target.
 *
 ...
 * uniqueness key is the RAW appName. So two distinct bundles (e.g. "boss-roast" and
 * "boss--roast") can map to the same target Secret. Writing both would leave
 * whichever went last in place, with no record of the other having been
 * overwritten.
 *
 * Keep the first appName in ascending order per target and report the rest as
 * collisions, so the caller can alert on the data problem. Which of two colliding
 * bundles wins is arbitrary; what matters is that it is stable, since a deploy
 * picking a different one each run would flip the preview's credentials between
 * deploys.
 */
export function dedupeSecretRecordsByTarget(
    ...
    for (const [secretName, group] of byTarget) {
        const ordered = group.length > 1 ? [...group].sort((a, b) => a.appName.localeCompare(b.appName)) : group;
