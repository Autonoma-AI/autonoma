...
The shape below is the config document - the contract the API, the MCP tools and this deploy path all speak. It is stored relationally (one row per app, service, connection, hook, repository and setup task) and composed back into this document on read, so the document remains how the config is authored and described even though it is not how it is held.
