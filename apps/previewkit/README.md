...
| `resources`     | No       |         | **Ignored for user-authored config.** App containers request 250m CPU and 1Gi memory, with the memory limit set to the same 1Gi - what a container is promised is what it may use. CPU is never limited, so apps still burst freely. The field is still accepted so existing configs validate, but its `cpu`/`memory` values have no effect here - resource sizing is honored only for trusted, platform-authored config. |
...
| `resources`   | No       |         | **Ignored for user-authored config** (see app fields). Service containers request 100m CPU and 1Gi memory, limited to the same 1Gi. |
