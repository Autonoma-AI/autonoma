...
Measures the single-shot SDK-integration flow: given the planner's frozen artifacts, an agent
(`claude -p` on Bedrock) implements the Autonoma SDK endpoint in a client repo, and an agentic
judge compares what it produced against the client's real ("golden") integration.

## How a run works

```
checkout(sha)  ->  copy to sandbox  ->  git apply strip.patch  ->  commit clean baseline
   ->  boot setup  ->  drive `claude -p` over the sandbox  ->  extract agent/golden diffs
   ->  stage judge trees  ->  agentic judge  ->  verdict.json
```
...
The harness does no live validation of its own - the signal is the judge's code comparison plus
the agent's own discover/up/down validation, read from the captured transcript.
...
| `artifacts/` | the frozen planner-artifact set (`AUTONOMA.md`, `entity-audit.md`, `scenarios.md`, `recipe.json`) fed to the agent. |
