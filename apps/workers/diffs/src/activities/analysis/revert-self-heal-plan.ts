...
import { RevertPlan, TestSuiteUpdater } from "@autonoma/test-updates";
 ...
 * the classifier's rewrite and re-ran it, but it still failed on a healthy app, so the test is kept rather than removed
 ...
 *
 * The restore repoints the assignment at the pre-rewrite plan record (`RevertPlan`) rather than re-authoring its text,
 * so the snapshot ends up genuinely unchanged for this test - the `planId`-keyed change computations would otherwise
 * report it as modified with identical before/after plans. No re-run is queued (the loop is over), and the restored plan
 * pins its own scenario. Row-local by construction. Returns `reverted: false` when the slug has no assignment.
 */
export async function revertSelfHealPlan(input: RevertSelfHealPlanInput): Promise<RevertSelfHealPlanOutput> {
    ...
    await updater.apply(new RevertPlan({ testCaseId: target.testCaseId, planId }));
    logger.info("Restored the pre-self-heal plan; no re-run queued", { extra: { testCaseId: target.testCaseId } });
