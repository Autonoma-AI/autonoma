...
|---|---|---|---|---|---|
| M1 | `## Start from the Autonoma UI` | The agent-first PREVIEW step: pairing code, the two terminal commands, idle cube, and the demoted "Answer a few questions instead" link | content area, nav cropped - the frame contains **both paths**, which is the point. `1120x855` (shoot at the viewport height rather than cropping - `sips -c` crops from the centre and eats the panel's top border) | `onboarding-previewenvironmentstep--agent-first` | [x] `configure-preview-ui.png` |
...
| M3 | `## How the agent sets Autonoma up` (end, beside the read-only paragraph - **not** replacing `configure-preview-flow.jpg`) | Read-only agent screen: Take over, live tool-call stream (`pair`, `get_config`, `apply_config`), chosen path, config so far, deploy status | `960x660` crop - the Preview path card made it taller than the old `535` | `onboarding-agentconfiguringscreen--configuring` | [x] `agent-configuring.png` |
