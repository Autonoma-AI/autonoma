...
- **It refuses rather than guesses** when a choice would be arbitrary - two coding agents installed and no `--coding-agent`, several frontends and no `--frontend`. A wrong guess spawns the wrong tool and burns a long run, so it fails with a clear error instead.
