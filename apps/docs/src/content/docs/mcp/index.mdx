...
**A tool says no preview environment was found.** The tools work against a PR that has a preview environment in your connected repo. Open the PR and let its preview deploy first.

**Logs come back empty or a tool 404s on an old PR.** Preview logs are retained for about 30 days, and previews are torn down after testing. Re-run the preview to get fresh logs.
