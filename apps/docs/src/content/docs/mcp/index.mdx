...
| `get_app_instructions` | repo | The two standing instruction fields from the app's settings page - `customInstructions` (how to drive the app during a run) and `testScopeGuidelines` (what to test and how to judge it) - plus the `fingerprint` a write must quote back |
