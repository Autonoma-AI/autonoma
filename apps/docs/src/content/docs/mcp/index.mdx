...
<p class="lead">Autonoma runs an MCP server so your coding agent can read a pull request's investigation findings and live debugging data - what Autonoma flagged and why, deploy status, build and runtime logs, a diagnosis, and which secrets are missing - and fix it from inside your repo, the same way it fixes anything else.</p>
...
| `get_investigation` | repo, PR | Autonoma's findings for the PR's latest checkpoint - each finding's what-happened, root cause, evidence, suggested fix, and signed screenshot/clip URLs, plus suggested tests. Start here when Autonoma flagged bugs |
| `report_false_positive` | repo, PR, findingId, reason? | Flags a `get_investigation` finding as a likely false positive (not a real bug). A tracking signal for Autonoma to review - it does **not** change the check, re-run the analysis, unblock the merge, or hide the finding |
