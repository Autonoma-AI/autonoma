...
- **Use it for onboarding** - set an app up: pick how it gets its previews, configure and deploy them (or wire your own pipeline), validate your SDK endpoint, and get your scenario recipes provisioning. You start this from inside Autonoma, which gives your agent a pairing code. → [Set up a preview with a coding agent](/mcp/configure-preview)
- **Use it for debugging** - once Autonoma is reviewing your pull requests: read what it flagged and why, pull logs and deploy status, fix the cause in your repo, and ask for a re-check. No pairing code - your agent identifies the app from your git remote. → the rest of this page.
...
[mcp_servers.autonoma]
command = "npx"
args = ["-y", "mcp-remote", "https://api.autonoma.app/v1/mcp", "--header", "Authorization: Bearer ${AUTONOMA_API_KEY}"]
```
...
That is the whole installation, and it covers both jobs. **Setting an app up?** [Set up a preview with a coding agent](/mcp/configure-preview) picks up from here: the pairing code that pins your app, how your agent chooses the preview path, and what it does on each. The rest of this page is the debugging job.
