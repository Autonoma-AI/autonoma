...
The same key works for the onboarding MCP, the REST API, and the planner CLI.
...
With no browser to complete that flow, pass an [API key](#headless-agents-and-ci) through the bridge instead:

```toml
# ~/.codex/config.toml
[mcp_servers.autonoma]
command = "npx"
args = ["-y", "mcp-remote", "https://api.autonoma.app/v1/mcp/debug", "--header", "Authorization: Bearer ${AUTONOMA_API_KEY}"]
```
