...
| `edit_previewkit_config` | repo/appId, PR, app, fields | Changes structural config for one service (path, Dockerfile, port, health check, build-secret keys, connections) and rebuilds it. Only the fields you pass change; never sets a secret value. Saved to the app's one config, so it applies to every environment |
