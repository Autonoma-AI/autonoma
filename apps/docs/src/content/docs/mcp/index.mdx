...
| `dry_run_scenario` | repo, scenarioId, recipe?, save? | Runs a recipe against your deployed SDK (`up` then `down`). Pass a `recipe` to try an edit **without storing it**; pass `save: true` to promote it, which happens only if the run passes |
| `update_recipe` | repo, scenarioId, recipe | Saves a recipe as the active one. Validated on save and rejected with the exact problems. Affects every future run of that scenario |
