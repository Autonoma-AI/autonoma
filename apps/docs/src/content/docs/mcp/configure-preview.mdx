...
4. **Watch and iterate** - `get_session_status` returns the deploy status, the preview URL, and diagnostics. The agent polls it, reads the diagnostics if the preview fails, fixes the config, and deploys again - until it comes up.
