...
2. **Follow the loop** - `select_preview_path` → `get_config` / `apply_config` → `trigger_deploy` → `get_session_status` (poll until ready) → verify the app works → `finish_onboarding` → `validate_sdk` → fix recipes with `dry_run_scenario` / `update_recipe`.
