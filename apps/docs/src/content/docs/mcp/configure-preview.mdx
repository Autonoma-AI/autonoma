...
4. Tell your agent: `configure my preview with the Autonoma MCP, code <your code>`. Name the MCP explicitly - an agent with several MCPs connected cannot tell which one "configure my preview" means, and will guess.
...
| `list_dry_run_targets` | applicationId | The previews a dry run can be pointed at - open PR previews and main, with which one Autonoma detected as the SDK implementation PR |
