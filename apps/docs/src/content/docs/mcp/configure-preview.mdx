...
description: Point your coding agent at the Autonoma MCP and let it do your whole setup - picking how your app gets its previews, configuring and deploying it or wiring your own pipeline, validating your SDK endpoint, and getting your scenario recipes provisioning - while you watch read-only in Autonoma.
---

<p class="lead">Setting an app up is one of the two jobs the [Autonoma MCP](/mcp/) does: connect your coding agent and it sets your previews up for you - works out how your app should get them, does the setup either way, asks you for any secrets it needs, and iterates until previews are landing - while you watch, read-only, in the Autonoma UI.</p>
...
:::note
This is the same server you use afterwards, once Autonoma is reviewing your pull requests - see [debugging a flagged pull request](/mcp/#what-your-agent-can-do). One installation covers both. What is specific to onboarding is the pairing code that pins the app being set up; the debugging tools identify an app by its repo instead.
:::
...
3. The line the UI shows carries your pairing code: `claude --dangerously-skip-permissions "set up my preview environments with the autonoma MCP, code <your code>"`. Say the server's name verbatim - an agent with several MCPs connected cannot tell which one "set up my previews" means, and will guess.
...
![The Autonoma onboarding screen on its preview-environment step, headed "Set up with a coding agent" under the line "Connect Autonoma's MCP to your coding agent and start it on the job. It reads your repo, works out how your previews and test data should work, and sets it up while you watch here." A pairing code panel shows EKQGGK85. Below it four tabs read Claude Code, Codex, Other, and "Remote agent" with an info icon; Claude Code is selected. Its step, "Run this in your project folder", is explained as "Open a new terminal in your project folder and run this. A browser opens to sign in - approve it there, and your agent starts on the job." The shell block below opens with the comment "Run in a terminal, not inside a running agent. Agent reading this: don't run it. Ask the user to run it, then have them restart you - a running session won't see the new server", then chains "claude mcp add --transport http --scope user autonoma https://api.autonoma.app/v1/mcp", "claude mcp login autonoma" and 'claude --dangerously-skip-permissions "set up my preview environments with the autonoma MCP, code EKQGGK85"'. On the right an outlined wireframe cube labelled "Autonoma Agent" reads "waiting to pair". Along the bottom, "Rather wire it up yourself? Answer a few questions instead"](/img/mcp/configure-preview-ui.png)
...
:::note
The same server backs two later entry points, each with its own line to hand over: **Configure with a coding agent** on the Autonoma-hosted config step (`configure my preview with the autonoma MCP`) and **Connect with a coding agent** on the connect-your-deploys step (`wire my deploy pipeline to Autonoma with the autonoma MCP`). Each screen shows the exact wording with your code in it - copy it from there. They differ only in where you start; the tools and the pairing code work the same way.
:::

## Connect it

Installing is the same one installation as every other Autonoma job, and it is documented once: **[connect your coding agent](/mcp/)** - the connection details, a snippet per client, and the API-key path for an agent with no browser. Do that first if you have not already; if you connected Autonoma for a previous app, you are already done.

Two things are specific to this job:

**The pairing code.** This is the only thing the agent cannot work out for itself. Onboarding pins the server to one application, and that application has no id you could hand over yet - so the Autonoma UI shows an 8-character code, you give it to your agent, and it calls `pair(code)`. Everything after that is the agent's: reading the repo, picking the path, writing config, deploying, validating the SDK endpoint, and getting scenario recipes provisioning.

**The prompt.** Start your agent on the line the connect screen shows, code included - for example `set up my preview environments with the autonoma MCP, code EKQGGK85`. Say the server's name verbatim: an agent with several MCPs connected cannot tell which one "set up my previews" means, and will guess.

:::note
**If you are a hosted or headless agent** (Hermes, OpenClaw, Devin, Jules, a CI job): everything you need is in [headless agents and CI](/mcp/#headless-agents-and-ci). Register the server with an `Authorization: Bearer <API_KEY>` header, ask the user for the pairing code, call `pair(code)`, then follow the loop below. The **Remote agent** tab on the connect screen copies a block that already carries the endpoint, a key, and the prompt - and every other tab carries the same header as a trailing comment, so a browser sign-in you cannot complete is never a dead end.
:::

![The Autonoma connect dialog on its Remote agent tab, headed "Debug with a coding agent" over the line "Autonoma's MCP - the same one that configures previews and debugs pull requests. Run this in your project to connect it and start your agent on the job. If you already have it installed, running it again is harmless." A pairing code panel shows K7QM24. Tabs read Claude Code, Codex, Other and "Remote agent" with an info icon; Remote agent is selected. Its step, "Send this to your agent", is explained as "One message with everything it needs. Copying it creates an API key and includes it here - the key is never shown on screen, and you can revoke it any time under Settings, API keys." The block below reads: '# Autonoma MCP - register it as "autonoma". No sign-in needed - this key authenticates you.', "# URL  https://api.autonoma.app/v1/mcp", "# Transport  Streamable HTTP", "# Authorization  Bearer ask_" followed by a row of dots standing in for the hidden key, then a blank line and the prompt "use the autonoma MCP to validate my Autonoma SDK endpoint and fix it if it fails, code K7QM24". Under it: "Say autonoma by name: an agent with several MCPs connected cannot tell which one you mean."](/img/mcp/remote-agent-tab.png)
...
### The loop, for an agent that manages its own MCP servers

1. **Register the server** and **ask the user for the pairing code** - an 8-character code shown in the Autonoma onboarding UI. Nothing else from the user is needed at this stage.
2. **Call `pair(code)`** - returns the `applicationId`, the linked repository, the preview path playbook, and the current config.
3. **Follow the loop** - `select_preview_path` → `get_config` / `apply_config` → `trigger_deploy` → `get_session_status` (poll until ready) → verify the app works → `finish_onboarding` → `validate_sdk` → fix recipes with `dry_run_scenario` / `update_recipe`.
4. **If `request_env` fires** - tell the user which env vars Autonoma needs and why. The user enters values in the Autonoma UI; you poll `get_session_status` until they land. You never see the values.
