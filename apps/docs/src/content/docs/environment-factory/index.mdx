...
:::note
The Environment Factory is what lets Autonoma provision real test data. It is a step of onboarding, immediately after **PR reviews** - preview environments and PR reviews start working there, and the flow then continues here rather than ending.
:::

## Get started

:::tip[Let the planner wire it for you]
You don't have to build this by hand. The [test planner](/test-planner/)'s "Set up test data" step hands the whole integration to your locally installed coding agent: it implements the endpoint, registers factories that call your real creation code, and validates the full up/down cycle against your running app - before any tests are written. The guides below are for doing it yourself, and for understanding what the agent set up.
:::
