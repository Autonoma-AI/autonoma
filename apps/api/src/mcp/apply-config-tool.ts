...
export interface ApplyConfigToolDeps {
    services: Services;
...
/**
 * `apply_config`: save the application's one preview-config document, with `prNumber` choosing
 * whether a pull request's environment is redeployed with it.
 *
 * What `prNumber` does NOT select is what gets written: an Application has a single
 * `PreviewkitConfig` row that every environment deploys from, so a PR-scoped preview config is
 * not representable. The tool's description says so outright, because the parameter shape reads
 ...
 */
export function registerApplyConfigTool(
    server: McpServer,
    { services, analytics, resolveTarget, guard }: ApplyConfigToolDeps,
) {
    server.registerTool(
        "apply_config",
        {
