...
8. finish_onboarding(applicationId) - take the app live. Selecting a deployment advances onboarding but does not finish it, and until it is finished Autonoma reviews no pull requests and holds back the comments it would have posted.
...
Your tools here: get_vercel_setup, link_vercel_project, create_vercel_deployment, get_vercel_deployment_status, select_vercel_deployment, finish_onboarding, get_session_status (for the preview URL and your control state - it carries no build logs on this path), plus the SDK and scenario/recipe tools once a deployment is selected.
